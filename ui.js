...
export function attachLogClick(view, els, rerenderAll){
  if (!els?.logBody) return;
  els.logBody.addEventListener('click', (e)=>{
    const row = e.target?.closest?.('.logEntry');
    if (!row || !view?.state) return;

    const org = row.dataset.org;
    const mi = row.dataset.mi;
    const part = row.dataset.part;

    if (!part && !mi) return; // nothing to highlight

    const orgN = (org === "" || org === undefined) ? -1 : (parseInt(org,10) || -1);
    const miN = (mi === "" || mi === undefined) ? null : (Number.isFinite(parseInt(mi,10)) ? parseInt(mi,10) : null);
